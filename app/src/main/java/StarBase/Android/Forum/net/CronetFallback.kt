// Adapted from LsbViewer https://github.com/Mei-Nagano/LsbViewer-main
// Copyright (c) 2026 Mei-Nagano, licensed under MIT License
// https://github.com/Mei-Nagano/LsbViewer-main/blob/main/LICENSE
...
/**
 * TCP/TLS fails → Cronet (QUIC/HTTP3) fallback transport.
 *
 * QUIC availability depends on current network and server; not treated as guaranteed proxy.
 * Only safe-to-replay or not-yet-sent requests can be retried.
 *
 * DNS: Cronet's built-in resolver hits system DNS (poisoned results fail QUIC cert checks),
 * so we MAP target domains to real IPs already resolved by this app's DoH via HostResolverRules.
 * Re-resolve on each redirect. Engine isolated per network-policy, domain, port, and IP.
 */
...
/** Retain request handle and poll upper-layer cancel; all exception paths cancel underlying request. */
        ...
        while (true) {
            ...
            try {
                return future.get(minOf(left, TimeUnit.MILLISECONDS.toNanos(150)), TimeUnit.NANOSECONDS)
                    .also { completed = true }
            } catch (_: TimeoutException) { /* check cancel & total deadline */ }
    ...
    private fun executeWithCronet(request: Request, call: okhttp3.Call): Response {
        ...
        while (true) {
            ...
            for (address in cronetAddressCandidates(addresses)) {
                ...
                val lease = CronetTransport.acquire(current.url.host, current.url.port, address.hostAddress!!, policy, context)
                try {
...
internal object CronetTransport {
    ...
    @Synchronized fun acquire(host: String, port: Int, ip: String, policy: String, context: android.content.Context): Lease {
        ...
        val entry = engines[key] ?: run {
            ...
            val engine = ExperimentalCronetEngine.Builder(context.applicationContext)
                .enableQuic(true)
...
internal fun withDefaultBrowserHeaders(request: Request): Request {
    ...
    if (request.header("User-Agent") == null) builder.header("User-Agent", Net.userAgent())
